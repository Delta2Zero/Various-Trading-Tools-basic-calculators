        ...
        while (!validInput) {
            ...
            for (size_t i = 0; i < rules.size(); ++i) {
                ...
                if (marketPrice >= rule.minPrice && marketPrice <= rule.maxPrice) {
                    if (numShares % rule.lotSize == 0) {
                        lotValid = true;
                        break;
                    } else {
                        minLotSize = rule.lotSize; // Set the minimum lot size for this price range
                    }
                }
            }

            if (!lotValid) {
                std::cout << "Odd lot detected! Please enter a number of shares that is a multiple of the minimum lot size (" << minLotSize << ").\n";
            } else {
                validInput = true;
            }
        }
